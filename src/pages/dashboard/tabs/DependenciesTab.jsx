    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 270px", gap: 12 }}>
      <div className="card">
        ...
        >
          ...
          <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
            {["all", "outdated", "unused", "security", "dev", "runtime"].map(
              (filter) => (
                <button
                  key={filter}
                  className="bg"
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Package", "Current", "Latest", "Type", "Usage", "Risk"].map(
                  (header) => (
                    <th
                      key={header}
                      style={{
                ...
                >
                  ...
                  >
                    <span
                      className="badge"
                      style={{
                        background: dep.unused
                          ? "rgba(248,113,113,.1)"
                          : "rgba(52,211,153,.1)",
                        color: dep.unused ? "#F87171" : "#34D399",
                        border: dep.unused
                          ? "1px solid rgba(248,113,113,.25)"
                          : "1px solid rgba(52,211,153,.2)",
                      }}
                    >
                      {dep.unused ? "unused" : "used"}
                    </span>
                  </td>
      ...
      <div className="card" style={{ height: "fit-content" }}>
        ...
        {[
          ...
          {
            label: "Unused",
            value: displayDeps.filter((dep) => dep.unused).length,
            color: "#F87171",
          },
        ].map((item) => (
          <div
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      Usage:{" "}
                      <span
                        style={{ color: dep.unused ? "#F87171" : "#34D399" }}
                      >
                        {dep.unused ? "unused" : "used"}
                      </span>
                    </div>
                  ...
                  {Array.isArray(dep.usageFiles) &&
                  dep.usageFiles.length > 0 ? (
                    <div
                      style={{
                        marginTop: 8,
                        borderTop: "1px solid #111D2E",
                    ...
                    >
                      ...
                      {dep.usageFiles.slice(0, 4).map((filePath) => (
                        <div
                          key={`${dep.name}-${filePath}`}
                          style={{
                            fontSize: 10,
                            color: "#94A3B8",
                            marginBottom: 3,
