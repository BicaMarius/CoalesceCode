...
export default function DiagramInspector({
  ...
  const selectedDetails = selectedNode
    ...
    : null;
  const definitionFiles = Array.isArray(selectedDetails?.definitionFiles)
    ? selectedDetails.definitionFiles
    : [];
  const usageFiles = Array.isArray(selectedDetails?.usageFiles)
    ? selectedDetails.usageFiles
    : [];
  const usageFrontendFiles = Array.isArray(
    selectedDetails?.usageByArea?.frontend,
  )
    ? selectedDetails.usageByArea.frontend
    : [];
  const usageBackendFiles = Array.isArray(selectedDetails?.usageByArea?.backend)
    ? selectedDetails.usageByArea.backend
    : [];
  const usageSharedFiles = Array.isArray(selectedDetails?.usageByArea?.shared)
    ? selectedDetails.usageByArea.shared
    : [];
  const impactPaths = Array.isArray(selectedDetails?.impactPaths)
    ? selectedDetails.impactPaths
    : [];
  const showDbTools = selectedNode?.id === "db";
  const showAiTools = selectedNode?.id === "ai";

  const renderFileSection = (title, files, color = "#94A3B8") => {
            ...
            <>
              ...
              {impactPaths.length ? (
                ...
                >
                  ...
                  {impactPaths.slice(0, 8).map((filePath) => (
                    <div
                      key={`${selectedNode.id}-impact-${filePath}`}
                      style={{
                        fontSize: 10,
                        color: "#E2E8F0",
                        marginBottom: 3,
              ...
              {renderFileSection("Used In Shared", usageSharedFiles, "#94A3B8")}
