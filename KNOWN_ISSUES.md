...
**Actual behavior:** Benchmark quality improved (`16 pass / 10 partial / 0 fail`), but large monorepos can still return partial core detection when signals are split across many packages/services.

**Workaround:** Re-run with GitHub token, inspect Analysis Log, and use benchmark comparison report (`docs/benchmarks/comparison-latest.md`) for rule-priority tuning.
...
3. Check `docs/benchmarks/latest.json` / `latest-summary.md` overwritten with transient fail-heavy output

**Expected behavior:** Latest benchmark artifacts should represent a valid full run.
**Actual behavior:** Rate-limited reruns can overwrite `latest.*` with non-comparable results.

**Workaround:** Provide `GITHUB_TOKEN`; when a rerun is rate-limited, restore `latest.*` from the last valid snapshot in `docs/benchmarks/results/`.
**Root cause:** Client-side benchmark depends directly on GitHub API quota and currently always overwrites `latest.*` outputs.
**Fix target:** v0.1.x
